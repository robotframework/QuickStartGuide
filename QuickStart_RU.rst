...
варьировать тестовые данные без дублирования выполняемых действий. С помощью настройки
...
`setups and teardowns`_ (рассматривается ниже). В нашем случае было бы проще создать отдельные
тесты со своими названиями для невалидной длины пароля и для сценариев с другими невалидными
данными. Однако, тогда потребовалось бы вынести эти тесты в отдельный файл, поскольку иначе
шаблон был бы применен и к другим тестам в этом файле.
...
Коллекция из нескольких тестовых сценариев называется в Robot Framework тестовым набором.
...
Аналогично, вы может использовать настройки `Suite Setup` и `Suite Teardown` для запуска
...
посредством настроек `[Setup]` и `[Teardown]` в таблице настроек тестового сценария.
...
также задавать метки для отдельных тестов используя настройку `[Tags]`, как это показано
...
Когда вы обратитесь к отчету после выполения тестов, вы увидете, что тесты получили
...
продублирован ниже. Глядя на код вы сможете увидеть, например, как ключевое слово
`Create User` соотносится с реализацией метода `create_user`.
